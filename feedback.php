<?php
	...
	function ask_feedback($cid,$pt,$person_name1,$person_name2,$email,$templ){
		...
		$mail->Username   = "vslbogd3an@gmail.com";  // GMAIL username
		...
		if ($count == 0) die('No feedback letter template found in database.');
			...
			for ($i = 0; $i < $bcount; $i++) {
				...
				if (($row['l_feed'] == 1) || ($row['l_feed'] == 3) || ($row['l_feed'] == 7))
					ask_feedback($row['con_id'],1,$row['leader'],$row['seeker'],$row['leader_email'],$templ);
					//$res .= $row['leader_id'] . " " . $row['leader'] . " | " . $row['leader_email'] . "<br>";
				if (($row['n_feed'] == 1) || ($row['n_feed'] == 3) || ($row['n_feed'] == 7))
					ask_feedback($row['con_id'],2,$row['seeker'],$row['leader'],$row['seeker_email'],$templ);
					//$res .= $row['seeker_id'] . " " . $row['seeker'] . " | " . $row['seeker_email'] . "<br>";				
			}
			echo 'sent. errors: ' . $mistakes;
/*				$person_name1 = 'YYYYY';
				$person_name2 = 'ZZZZZ';
				$message = $row['text'];
...
*/			
	...
	/*		return $mail_result;
		...
		$query = "update connections as con set con.l_feed=" . ($_GET['feed'] == 'good' ? 2 : 1) . ", con.l_feed_date=NOW() where con.id=" . $_GET['id'];
